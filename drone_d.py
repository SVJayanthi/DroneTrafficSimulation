# -*- coding: utf-8 -*-
"""
Created on Thu Mar 14 21:48:51 2019
    ...
    def step(self, dt):
        ...
        for i1, i2 in zip(ind1, ind2):
            ...
            if (m < (self.size * 20)):
                ...
                self.state[i1, 6:8] += rem * self.acc_hor * dt
                self.state[i1, 0:2] += 0.5 * u * self.acc_hor * dt
                self.state[i2, 6:8] -= rem * self.acc_hor * dt
                self.state[i2, 0:2] -= 0.5 * u * self.acc_hor * dt
...
def state_d(i):
    box = ParticleBox(drones = i + 1)
    length = m.trunc(400 * m.log(i+40))
    for j in range(length):
        box.step(1)
    print(i)
    with open('drones_drone_new.csv', 'a') as writeFin:
...
if __name__ == '__main__':        
    ...
    pool.map(state_d, range(1000))
    pool.close()
    print("completo")

"""    processes = [ ]
    for i in range(400):
        p = multiprocessing.Process(target=state_d, args=(i,))
        processes.append(p)
        p.start()

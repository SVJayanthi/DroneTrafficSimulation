# -*- coding: utf-8 -*-
"""
Created on Thu Mar 14 21:51:31 2019
...
def state_d(i):
    wind_speed = np.random.standard_normal(3)
    wind_speed -= 0.5
    n = np.linalg.norm(wind_speed)
    wind_speed /= n
    wind_speed *= (i/2)
    box = ParticleBox(wind = wind_speed)
    length = m.trunc(400 * m.log(i+40))
    for j in range(length):
        box.step(1)
    print(i)
    with open('drones_wind.csv', 'a') as writeFin:
